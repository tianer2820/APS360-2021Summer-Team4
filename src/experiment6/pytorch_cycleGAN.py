...
def train(configures, proj_name, proj_group, test_per_epoch=10, save_per_epoch=50, continue_train=False, pretrain=False):
    ...
    with wandb.init(project=proj_name, group=proj_group, config=configures):
        ...
        if pretrain:
            dataset_A = get_dataset('data_horse2zebra/trainB/', config.img_size, use_normalize=config.normalize_img)
            dataset_B = get_dataset('data_horse2zebra/trainA/', config.img_size, use_normalize=config.normalize_img)
        else:
...
if __name__ == "__main__":
    ...
    train(configures=config, proj_name='cycleGan_test',
          proj_group='experiment6', test_per_epoch=10, save_per_epoch=50, continue_train=CONTINUE_TRAINING, pretrain=PRETRAIN)
