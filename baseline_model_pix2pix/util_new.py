...
def show_result(G, x_, num_epoch, show = False, save = False, path = 'result.png'):
    ...
    size_figure_grid = 2
    fig, ax = plt.subplots(x_.size()[0], size_figure_grid, figsize=(size_figure_grid, x_.size()[0]), squeeze=False)
...
def imgs_resize(imgs, resize_scale = 286):
    ...
    for i in range(imgs.size()[0]):
        ...
        np_img = imgs[i].numpy()
        np_img = (np_img + 1) * (255 / 2)
        np_img: np.ndarray
        np_img = np_img.clip(0, 255)
        np_img = np_img.astype(np.uint8)
        img = Image.fromarray(np_img, mode='RGB')
        img_resized = img.resize([resize_scale, resize_scale])
        new_img = np.array(img_resized)
        outputs[i] = torch.FloatTensor((new_img.transpose(2, 0, 1).astype(np.float32).reshape(-1, imgs.size()[1], resize_scale, resize_scale) - 127.5) / 127.5)
